...
// ============================================
// Анализирует стиль текста пользователя через Claude
...
async function analyzeToneOfVoice(exampleText) {
  ...
  try {
    console.log('🔍 Анализирую Tone of Voice...');
...
Верни ТОЛЬКО JSON в формате:
{
  ...
  "language_level": "simple/intermediate/advanced"
    ...
    console.log('✅ ToV профиль получен:', tovProfile);

    return tovProfile;

  } catch (error) {
    console.error('❌ Ошибка анализа ToV:', error.response?.data || error.message);
    return getDefaultTovProfile();
  }
}
