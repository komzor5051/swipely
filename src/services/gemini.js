...
function buildSystemPrompt(designConfig, slideCount, toneGuidelines, contentTone) {
  ...
  const slideStructures = {
    ...
    7: `1. hook — мгновенная остановка скролла\n2. tension — усиление боли или проблемы\n3. value — конкретная польза или причина\n4. accent — шокирующая цифра или прямой вызов\n5. value — продолжение с примером\n6. insight — неожиданный вывод\n7. cta — одно простое действие`,
    9: `1. hook\n2. tension\n3. value — польза #1\n4. accent — триггер, шокирующая цифра\n5. value — польза #2\n6. value — польза #3\n7. accent — провокационный вызов читателю\n8. insight — неожиданный вывод\n9. cta`,
    12: `1. hook\n2. tension\n3. value — польза #1\n4. accent — триггер #1\n5. value — польза #2\n6. value — польза #3\n7. accent — триггер #2\n8. value — польза #4\n9. value — польза #5\n10. accent — триггер #3\n11. insight — неожиданный вывод\n12. cta`,
  };
  const slideStructure = slideStructures[slideCount] || slideStructures[7];
...
ЗАПРЕЩЕНО В ТЕКСТАХ:
- Конструкция "не X, а Y" и любые вариации: "это не X — это Y", "не копируй, а создавай"
- Мягкие слова: попробуй, возможно, стоит рассмотреть, можно подумать
- Абстракции без расшифровки: стратегия, подход, система, контент (без конкретики)
- Клише: найди себя, выйди из зоны комфорта, а вы знали, это меняет всё
...
ТИПЫ СЛАЙДОВ:
- hook — мгновенная остановка скролла
- tension — усиление боли или проблемы
- value — конкретная польза
- accent — резкая смена темпа: шокирующая цифра, неожиданное сравнение, прямой вызов читателю или провокационный вопрос. title: дерзкое короткое заявление. content: 1-2 предложения максимум, бьёт как удар.
- insight — неожиданный вывод
- cta — одно простое действие

СТРУКТУРА:
${slideStructure}

Первый слайд (hook) — формулы заголовка (использовать одну):
- Провокация + специфика: конкретное заявление, которое ломает ожидание
- Число + результат: цифра + что она означает для читателя
- Перевёрнутое ожидание: то, что думают все + неожиданный разворот
ЗАПРЕЩЕНО в заголовке первого слайда: абстрактные слова без объяснения, вопрос без намёка на ответ, просто 1-2 слова без контекста.
...
ПРИМЕР (3 слайда — масштабируй на ${slideCount}):
{
  "slides": [
    ...
    {"type": "accent", "title": "<hl>47 часов</hl> до отказа", "content": "Столько ждут ответа. За это время конкурент уже закрыл сделку."},
