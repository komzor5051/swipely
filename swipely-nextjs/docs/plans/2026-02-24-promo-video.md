# PhonePromo — Remotion 60s Promo Video

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Implement `PhonePromo.tsx` — 60-секундная вертикальная (1080×1920, 9:16) Remotion-композиция для Instagram Reels/Stories. Текстовый крюк → iPhone mockup влетает → 4 демо-сегмента внутри телефона → outro.

**Architecture:** Единый файл `PhonePromo.tsx` со всеми sub-компонентами. Timing через frame-boundary константы (`F`). Все "screen" компоненты рендерятся внутри `<PhoneMockup>` с `overflow: hidden`. Crossfade между сегментами через `interpolate()` opacity. `Sequence` для логической группировки.

**Tech Stack:** Remotion 4.x, React 18, TypeScript. Шрифты: `@remotion/google-fonts/Outfit` + `@remotion/google-fonts/SpaceMono` (уже используются в `LaunchPromo.tsx`).

---

### Task 1: Register PhonePromo composition in Root.tsx

**File:** `swipely-promo/src/Root.tsx`

**Step 1: Добавить import после существующих**

```tsx
import { PhonePromo } from "./PhonePromo";
```

**Step 2: Добавить Composition после LaunchPromo**
...
**Step 3: Проверить в Remotion Studio**

```bash
cd "/Users/lvmn/Desktop/Бизнес/ai projects /swipely /swipely-promo" && npm run studio
```

Ожидаем: `PhonePromo` появляется в списке (ошибка импорта — нормально, файл ещё не существует).

---

### Task 2: Create PhonePromo.tsx skeleton

**File:** `swipely-promo/src/PhonePromo.tsx` (новый файл)
...
// Placeholder export — sub-components added in later tasks
export const PhonePromo: React.FC = () => {
  const frame = useCurrentFrame();

  return (
    <AbsoluteFill style={{ backgroundColor: INK, fontFamily: bodyFont }}>
      {/* Sub-components will be added here */}
    </AbsoluteFill>
  );
};
```

**Step 2: Проверить в Remotion Studio**

Композиция открывается как сплошной `#0D0D14` экран без ошибок.

---

### Task 3: TextIntro component (Scene 1, fr 0–120)

Добавить `TextIntro` и хелпер `useSlideUp` перед экспортом `PhonePromo` в `PhonePromo.tsx`.
...
**Step 2: Рендерить в корневом PhonePromo**

```tsx
// Scene 1 fade-out at fr 105–120
...
**Step 3: Проверить в Studio**

Скрабнуть fr 5 (первая строка), fr 20 (все строки), fr 65 (subtitle), fr 110 (fade out).

---

### Task 4: PhoneMockup + LimeRect components
...
### Task 5: PhoneContainer — spring-in animation (fr 120–1800)
...
**Step 2: Добавить в PhonePromo root**

```tsx
{frame >= F.PHONE_IN && (
  <PhoneContainer>
    {/* Demo segments added in Tasks 6–9 */}
  </PhoneContainer>
)}
```
...
fr 130: телефон появляется снизу. fr 150: lime rect появляется. fr 240: телефон на месте.

---

### Task 6: DemoInput component (Segment A, fr 240–630)
...
**Step 2: Добавить в PhoneContainer**
...
### Task 7: DemoTemplates component (Segment B, fr 630–1020)
...
const DemoTemplates: React.FC = () => {
  ...
  // Lime outline cycles through cards: 60fr per card
...
**Step 2: Добавить к DemoInput внутри PhoneMockup**

Оба компонента управляют своей opacity — видим только один в каждый момент.

---

### Task 8: DemoGenerating component (Segment C, fr 1020–1410)
...
### Task 9: DemoResult component (Segment D, fr 1410–1800)
...
const DemoResult: React.FC = () => {
  ...
  // Swipe transitions
  const swipe1 = interpolate(frame, [1500, 1530], [0, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp", easing: Easing.inOut(Easing.cubic) });
  const swipe2 = interpolate(frame, [1600, 1630], [0, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp", easing: Easing.inOut(Easing.cubic) });
  const currentSlide = swipe1 + swipe2;
  const trackX = -currentSlide * 316; // 316 = phone screen inner width
...
### Task 10: SceneHeadline component
...
### Task 11: OutroOverlay + сборка финального PhonePromo
...
**Step 2: Финальный PhonePromo root**

Заменить placeholder на полную сборку:
...
### Task 12: QA checklist в Remotion Studio

Скрабнуть эти фреймы и проверить:

| Frame | Ожидаем |
|-------|---------|
| 5     | Первая строка "От текста" — slide up |
| 20    | Все три строки видны (stagger) |
| 65    | `swipely.ai` fade in |
| 110   | Всё гаснет (introOp → 0) |
| 130   | Телефон влетает снизу |
| 150   | Lime rect scale in за телефоном |
| 255   | DemoInput: textarea, toggle, lime кнопка |
| 400   | Мигающий курсор в textarea |
| 645   | DemoTemplates: сетка, первая карточка выделена |
| 705   | Вторая карточка выделена (+60fr) |
| 1035  | DemoGenerating: прогресс-бар ~0% |
| 1215  | Прогресс-бар ~50% |
| 1405  | Прогресс-бар ~97% |
| 1425  | DemoResult: первый слайд |
| 1515  | Swipe → второй слайд |
| 1615  | Swipe → третий слайд |
| 1755  | Outro overlay появляется |
| 1799  | Полный outro: лого + swipely.ai + lime CTA |
...
```bash
cd "/Users/lvmn/Desktop/Бизнес/ai projects /swipely /swipely-promo" && git add src/Root.tsx src/PhonePromo.tsx && git commit -m "feat(video): PhonePromo 60s vertical promo — text intro + phone demo"
```
...
```bash
cd "/Users/lvmn/Desktop/Бизнес/ai projects /swipely /swipely-promo" && npx remotion render PhonePromo output/phone-promo.mp4
```

Для быстрого превью:
```bash
npx remotion render PhonePromo output/phone-promo-preview.mp4 --scale 0.5
```
